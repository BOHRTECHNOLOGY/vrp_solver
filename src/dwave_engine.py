...
class DWaveEngine(object):
    ...
    def solve_for_all_partitions(self, problem, **params):
        """Solve given CVRP problem for all possible partitions.
    ...
    def _get_raw_solutions(self, problem, **params):
        ...
        # check for any unused (and therefore: invalid) parameters
        used_param_names = set(solver_params).union(set(qubo_params))
        for param_key in params:
            if param_key not in used_param_names:
                raise TypeError('{} is not a valid keyword argument.'.format(param_key))
...
def generate_partitions(outposts, vehicles):
    ...
    for combination in itertools.combinations(range(number_of_nodes+number_of_vehicles-1), number_of_vehicles-1):
        ...
        vehicles_partitions.add(current_partition)
