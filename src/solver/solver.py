...
def outposts_preprocessing(outposts, vehicles):
    ...
    # Filter out outposts with approx. zero load, e.g. when load
    # is evenly divisible by capacity
    updated_outposts = outposts[(outposts.load >= 0.001) | (outposts.outpost_id == 0)]
    return updated_outposts, vehicles_left, vehicles_used
...
def solve(datadir, solver_name='ortools', calculation_time=5, savedir='.',
    ...
    Args:
        ...
        calculation_time: suggested calculation time in seconds for single (day, time shift) pair
    ...
    if load_vehicles:
        log.info('Loading vehicles fleet from file...')
    ...
    log.info("Selecting solver: %s" % solver_name)
    solver = dict(SOLVERS)[solver_name]

    solutions = []
    suffixes = list(itertools.chain.from_iterable([['%dd' % i, '%dn' % i] for i in range(1, 8)]))
    for suffix in suffixes:
        outposts_path = os.path.join(datadir, "outposts_%s.csv" % suffix)
        log.info('Solving problem for: ' + outposts_path)
        outposts = pd.read_csv(outposts_path, sep=';')

        if limit_outposts is not None:
            log.warn('\033[93mWARNING! WE ARE SOLVING VRP PROBLEM ONLY FOR FIRST %d OUTPOSTS\033[0m' % limit_outposts)
        ...
        if  total_load > total_capacity:
            log.error('We are not able to handle all packages! Total load %.2f is greater than total capacity %.2f' % (total_load, total_capacity))
            exit(0)

        log.info('Preprocess outposts. Outposts length: %d' % len(outposts))
        updated_outposts, vehicles_left, vehicles_used = outposts_preprocessing(outposts, vehicles)
        log.info('Preprocessing done! Updated outposts length: %d' % len(updated_outposts))
        if updated_outposts is None:
        ...
        for outpost_id, vehicle_id, vehicle_capacity in vehicles_used:
            log.info('Outpost Id: {:4d} Vehicle Id: {:4d} Vehicle capacity: {:4.1f}'.format(outpost_id, vehicle_id, vehicle_capacity))

        log.info('Preprocessed outposts list:')
        log.info(updated_outposts[['outpost_id', 'outpost_name', 'load']])

        log.info('Building graph...')
        # Build graph at the end, when outposts loads are updated
        graph = read_graph_from_df(graph_df, updated_outposts)

        log.info('Running solver...')
        log.info('Max distance per route is: %.0f m' % max_route_distance)
        time_start = time.time()
        df_routes = solver.calculate_routes(updated_outposts, vehicles_left, graph, 
        ...
        if df_routes is not None:
            ...
            path = os.path.join(savedir, 'routes_%s.csv' % suffix)
            solutions.append((path, df_routes))
            log.info('Solution preview: \n' + str(df_routes))
            log.info(INFO.format(datadir=datadir, solver=solver_name, elapsed=elapsed,
                savepath=path))
        else:
            log.error('No solution found for: %s' % outposts_path)
            exit(0)
    os.makedirs(savedir, exist_ok=True)
    for path, df in solutions:
        df.to_csv(path, sep=';', index=False) 
    vehicles.to_csv(os.path.join(savedir, 'vehicles.csv'), sep=';')
...
`outposts.csv` file')
    ...
    solve(datadir=args.datadir, solver_name=args.solver, savedir=args.savedir,
        max_route_distance=args.max_distance * 1000, calculation_time=args.calc_time, 
        vehicles_list=args.gen_vehicles, limit_outposts=args.limit_outposts)
